...
Geometry::~Geometry()
{
	...
	if (m_uvwbuffer != INVALID_OGL_VALUE)  glDeleteBuffers( 1, &m_uvwbuffer);
...
void Geometry::createBuffers()
{
	...
	if ( m_uvwbuffer == INVALID_OGL_VALUE && m_uvw.size()>0)
	{
		glGenBuffers(1, &m_uvwbuffer);
		glBindBuffer(GL_ARRAY_BUFFER, m_uvwbuffer);
		glBufferData(GL_ARRAY_BUFFER, m_points * sizeof(glm::vec3), &m_uvw[0], GL_STATIC_DRAW);
	}
	...
	glBindBuffer(GL_ARRAY_BUFFER, m_uvwbuffer);
	glEnableVertexAttribArray(UVWCOORDS);
	glVertexAttribPointer(UVWCOORDS, 3, GL_FLOAT, GL_FALSE, 0, 0);
	...
	glDisableVertexAttribArray(UVWCOORDS);
