...
struct PushState {
    explicit PushState(GLenum e) : b(glIsEnabled(e)), e(e) {}
    ~PushState()
    {
        if (b) {
            glEnable(e);
...
void ImGui_ImplGlfw_RenderDrawLists(ImDrawData* draw_data)
{
    ...
    glEnableVertexAttribArray(static_cast<GLuint>(g->Position));
    glEnableVertexAttribArray(static_cast<GLuint>(g->TexCoord));
    glEnableVertexAttribArray(static_cast<GLuint>(g->Color));
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        const auto * vtx_buffer = (const unsigned char*)&cmd_list->VtxBuffer.front();
        const ImDrawIdx* idx_buffer = &cmd_list->IdxBuffer.front();
        glVertexAttribPointer(static_cast<GLuint>(g->Position), 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert), (void*)(vtx_buffer + OFFSETOF(ImDrawVert, pos)));
        glVertexAttribPointer(static_cast<GLuint>(g->TexCoord), 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert), (void*)(vtx_buffer + OFFSETOF(ImDrawVert, uv)));
        glVertexAttribPointer(static_cast<GLuint>(g->Color), 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ImDrawVert), (void*)(vtx_buffer + OFFSETOF(ImDrawVert, col)));
    ...
    glDisableVertexAttribArray(static_cast<GLuint>(g->Position));
    glDisableVertexAttribArray(static_cast<GLuint>(g->TexCoord));
    glDisableVertexAttribArray(static_cast<GLuint>(g->Color));
...
GLuint ImGui_ImplGlfw_CreateShader(GLenum shaderType, const char* shaderSrc, GLuint& shader)
{
    ...
    if (!compiled) {
        ...
        if(infoLen > 1) {
            msg.resize(static_cast<unsigned long>(infoLen));
            glGetShaderInfoLog(shader, infoLen, nullptr, &msg[0]);
        }
...
void ImGui_ImplGlfw_CreateProgram()
{
    ...
    if(!linked) {
        ...
        if(infoLen > 1) {
            msg.resize(static_cast<unsigned long>(infoLen));
            glGetProgramInfoLog(g->Program, infoLen, nullptr, &msg[0]);
        }
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    io.DeltaTime = g->Time > 0.0 ? (float)(current_time - g->Time) : 1.0f / 60.0f;
    g->Time = current_time;
